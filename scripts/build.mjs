...
function homePage() {
  const body = [
    ...
    "    <h1>十のセッションを、<br><em>リプレイ小説</em>で読む。</h1>",
    '    <p class="hero-copy">エモクロアTRPG『ロールシャッハシンドローム』。<br>配信セッションをもとに構成した十のリプレイ小説。</p>',
    ...
    '    <div class="section-heading"><p class="eyebrow">Replay collection</p><h2 id="works-title">収録作品</h2><p>十のセッションを、それぞれ一篇のリプレイ小説として収録しています。</p></div>',
    '    <div class="work-grid">' + works.map(card).join("") + "</div>",
    "  </section>",
    '  <section class="about"><div><p class="eyebrow">About</p><h2>セッションの声を、<br>読み物の時間へ。</h2></div><p>実際のセッションで交わされた台詞をできる限り残し、場面の空気や間、視線や心の動きとともに、一篇の小説として編み直しています。噛みや言い直しは読みやすく整えながら、発話が持つ情報や選択理由は削らない方針です。</p></section>',
    "</main>",
